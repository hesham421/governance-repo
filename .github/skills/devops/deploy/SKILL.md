...
## When NOT to Use
- For application source code changes (backend Java or frontend TypeScript) — use the appropriate backend or frontend skills
- For reviewing existing deployment configs without generating new artifacts
- For local development database setup — that is `backend/docker/docker-compose.yml` (separate from production)
- For CI/CD pipeline configuration (GitHub Actions) — that is a separate concern
...
- Complete deployment artifact set:
  - `backend/Dockerfile` (lives in the `backend` repo root)
  - `frontend/Dockerfile` (lives in the `frontend` repo root)
  - `frontend/nginx.conf` (lives in the `frontend` repo root)
...
| Check | How |
|-------|-----|
| Spring Boot version | Read `pom.xml` in the `backend` repo → `<parent>` → `spring-boot-starter-parent` version |
| Java version | Read `pom.xml` → `<java.version>` |
| Modules list | Read `pom.xml` → `<modules>` (skip commented-out modules) |
...
| Check | How |
|-------|-----|
| Angular project name | Read `angular.json` in the `frontend` repo → first key under `projects` |
...
| Environment file | Read `src/environments/environment.prod.ts` — verify `apiUrl: '/api'` (proxy-relative) |
...
**Path:** `backend/Dockerfile` (in the `backend` repository root)
...
**Path:** `frontend/Dockerfile` (in the `frontend` repository root)
...
# Copy custom nginx config (nginx.conf lives at frontend/ repo root)
COPY nginx.conf /etc/nginx/conf.d/default.conf

# Copy built Angular app
COPY --from=build /app/dist/n-erp-system /usr/share/nginx/html
...
**Path:** `frontend/nginx.conf` (in the `frontend` repository root)
...
| ID | Rule | ❌ Forbidden | ✅ Required |
|----|------|-------------|------------|
| DC.1 | Build context is repo root | `context: ..` from `deploy/` using monorepo root | `context: ../backend` or `context: ../frontend` (each repo root) |
...
| File | Purpose |
|------|---------|
| `backend/Dockerfile` | Multi-stage backend build (in `backend` repo) |
| `frontend/Dockerfile` | Multi-stage frontend build (in `frontend` repo) |
| `frontend/nginx.conf` | Nginx config with SPA fallback + API proxy to localhost (in `frontend` repo) |
| `deploy/docker-compose.yml` | Full-stack orchestration (host network, references sibling repos) |
...
|------|--------|-----------|
| `src/environments/environment.prod.ts` | Set `apiUrl: '/api'` and `authApiUrl: '/api'` | ALWAYS — proxy-relative is mandatory |
| `.gitignore` (in each repo) | Add `deploy/.env`, `docker-compose.override.yml`, `*.log.*` | If not already present |

### 7.3 Files the Skill Must NEVER Modify

- `pom.xml` (in backend repo) — read-only for analysis
- `angular.json` (in frontend repo) — read-only for analysis
- `package.json` (in frontend repo) — read-only for analysis
- Any source code files under `erp-*/src/` or `src/app/`
...
| Scenario | Adaptation |
|----------|------------|
| New module added to `pom.xml` (backend repo) | Add its POM + src COPY lines to backend Dockerfile |
...
| Inputs | `pom.xml` (backend repo), `angular.json` (frontend repo), `application-prod.properties`, `environment.prod.ts`, `.gitignore` |

...
## When NOT to Use
- Before the backend DTOs are defined and reviewed — frontend models must mirror backend contracts
- When models already exist and only one field needs updating (edit directly)
- For backend Java DTOs — use `create-dto` instead
- For shared `core/` or `shared/` models that span multiple features (those are manually curated)
...
- `src/app/modules/<DOMAIN_DIR>/<FEATURE_DIR>/models/<feature>.model.ts`
- `src/app/modules/<DOMAIN_DIR>/<FEATURE_DIR>/models/<feature>-form.model.ts`
...
### 1. Create Model File
- **Location:** `src/app/modules/<DOMAIN_DIR>/<FEATURE_DIR>/models/<ENTITY_KEBAB>.model.ts`
...
- **Location:** `src/app/modules/<DOMAIN_DIR>/<FEATURE_DIR>/models/<ENTITY_KEBAB>-form.model.ts`

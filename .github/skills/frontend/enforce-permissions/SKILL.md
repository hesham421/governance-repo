...
## When NOT to Use
- For pages or routes that are intentionally public (login page, error pages)
- For backend `@PreAuthorize` validation — that is part of `enforce-backend-contract`
- For `SecurityPermissions.java` constant definitions — those are managed separately
- When there are no user-action buttons or guarded routes in the reviewed code

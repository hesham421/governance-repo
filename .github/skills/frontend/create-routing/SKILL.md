...
## When NOT to Use
- Before the facade is defined — confirm action helpers depend on facade methods
- For public/unauthenticated pages that require no permission guards
- For modifying the root app routing (`app-routing.module.ts`) — that is a structural change requiring manual review
- For backend routing / `@RequestMapping` configuration — use `create-controller` instead
...
- `src/app/modules/<DOMAIN_DIR>/<FEATURE_DIR>/<feature>.routes.ts`
- `src/app/modules/<DOMAIN_DIR>/<FEATURE_DIR>/helpers/<feature>-confirm-actions.ts`
...
### File Location
`src/app/modules/<DOMAIN_DIR>/<FEATURE_DIR>/helpers/<ENTITY_KEBAB>-confirm-actions.ts`
...
### File Location
Add routes to `src/app/modules/<DOMAIN_DIR>/<DOMAIN>-routing.module.ts`

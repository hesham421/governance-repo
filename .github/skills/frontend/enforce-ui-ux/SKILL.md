...
## When NOT to Use
- For non-rendering files (guards, resolvers, pure services, interceptors)
- For backend code — use `enforce-backend-contract` instead
- For CSS/SCSS token validation — use `enforce-design-system` instead
- For code structure validation — use `enforce-frontend-architecture` instead

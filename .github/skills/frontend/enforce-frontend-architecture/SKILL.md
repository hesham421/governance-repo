...
## When NOT to Use
- For backend code — use `enforce-backend-contract` instead
- For governance/config files (`.github/`, `angular.json`, AI governance config) — those are not application code
- For unit test files — test architecture has separate conventions
- When validating a single specific concern (use the targeted skill: `enforce-permissions`, `enforce-state-management`, etc.)

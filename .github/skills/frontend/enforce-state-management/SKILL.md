...
## When NOT to Use
- For non-facade files (models, API services, components that hold no state)
- For backend code — use `enforce-backend-contract` instead
- For static presentation components that have no `signal()` or subscription
- When reviewing file structure concerns — use `enforce-frontend-architecture` instead

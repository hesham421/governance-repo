...
## When NOT to Use
- Before models are defined — `create-models` must run first
- When the API service already exists and only one method needs adding (edit directly)
- For direct `HttpClient` usage — all HTTP must go through `BaseApiService`
- For `shared/` or `core/` services that are not feature-specific API services
...
- Single file: `src/app/modules/<DOMAIN_DIR>/<FEATURE_DIR>/services/<feature>-api.service.ts`
...
- **Location:** `src/app/modules/<DOMAIN_DIR>/<FEATURE_DIR>/services/<ENTITY_KEBAB>-api.service.ts`

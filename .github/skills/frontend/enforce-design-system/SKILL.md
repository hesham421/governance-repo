...
## When NOT to Use
- For TypeScript or HTML template files — this skill covers SCSS only
- For backend code or deploy configuration
- For `erp-tokens.scss` additions without following the token definition rules (the skill explains those rules)
- For Mantis theme base styles — those are read-only third-party files

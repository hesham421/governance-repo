...
## When NOT to Use
- Before models and API service exist — both must be completed first
- When the facade already exists and only one method needs updating (edit directly)
- For `providedIn: 'root'` singleton services — facades are always component-scoped
- For backend service layer — use `create-service` instead
...
- Single file: `src/app/modules/<DOMAIN_DIR>/<FEATURE_DIR>/facades/<feature>.facade.ts`
...
- **Location:** `src/app/modules/<DOMAIN_DIR>/<FEATURE_DIR>/facades/<ENTITY_KEBAB>.facade.ts`

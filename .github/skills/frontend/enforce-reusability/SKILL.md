...
## When NOT to Use
- For the first occurrence of a pattern — no duplication exists yet to detect
- For backend code — this skill covers frontend only
- When explicitly creating a one-off helper that does not belong in shared code
- For `shared/` or `core/` files themselves — those are the targets of extraction, not subjects of this skill
